...
{
    public class SvCatalogoAlmacenConsultar
    {
        ...
        public SvCatalogoAlmacenConsultar(RpCatalogoAlmacen repositorio, EsCatalogoAlmacen esCatalogoAlmacen)
        {
        ...
        private void ObtenerEntidades()
        {
            ...
            IEnumerable<EnCatalogoAlmacen> Query = _rpCatalogoAlmacen._dbSet;
            ...
            if (FiltroPaginacion != null && FiltroPaginacion.B_Paginacion)
                Query = Query
                    .Where(x => x.AlmacenId > FiltroPaginacion.UltimoId)
                    .Take(FiltroPaginacion.NumeroRegistros);
            if (FiltroEntidad != null)
            {
                ...
                if (!string.IsNullOrEmpty(FiltroEntidad.IPServer))
                    Query = Query.Where(q => q.IPServer.ToUpper().Contains(FiltroEntidad.IPServer.ToUpper()));
            }
            if (FiltroConsulta != null)
            {
